...
export class ApiService {
  ...
  async translateToEnText(input: string) {
    try {
      const req = this.http.post(
        'https://api.cognitive.microsofttranslator.com/translate?api-version=3.0&from=lt&to=en',
        [
          {
            text: input,
          },
  ...
  async translateToLtText(inputs: string[]) {
    const req = this.http.post(
      'https://api.cognitive.microsofttranslator.com/translate?api-version=3.0&to=lt',
      inputs.map((input) => ({
        text: input,
      })),
      {
  ...
  async getSimilarWords(request: SearchWordRequest) {
    ...
    const req = this.http.get(
      `https://api.datamuse.com/words?rel_jjb=${mainWord}&max=200&md=f`,
    );
